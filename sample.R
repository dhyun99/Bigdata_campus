...
wifi = fread('wifi_dong.csv')
building = fread('building_five.csv')
dong_doundary = fread('dong_boundary.csv')
...
#p=1 일때의 좌표값
dobong1_coord_fin <- cbind(dobong1_coord_fin,X = rowSums(distm(dobong1_coord_fin[,c(1,2)],
                                                               dobong1_nowifi_build[,3:4], fun = distHaversine) <= 50))
